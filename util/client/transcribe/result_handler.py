...
class ResultHandler:
    ...
    @classmethod
    def save_results(
        cls, file: Path, message: Dict[str, Any], logger: Optional[Logger] = None
    ) -> str:
        """
        保存转录结果到文件
        ...
        _logger.debug(f"message keys: {message.keys()}")
        ...
        # 记录处理前后的变化
        _logger.debug(f"text_display: 处理后={text_display}")
        _logger.debug(f"text_accu: 处理后={text_accu}")
        # 文件名
