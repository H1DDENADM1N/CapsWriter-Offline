...
try:
    from mistral_common.tokens.tokenizers.mistral import (
        MistralTokenizer,  # pyright: ignore[reportMissingImports]
    )
...
class SpecialVocab:
    ...
    def _try_load_from_tokenizer_json(self, path: Path) -> bool:
                    ...
                    if processor.get("type") == "TemplateProcessing":
                        ...
                        if len(tmpl_single) > 1:
                            if (
                                ...
                                self.add_special_token["bos"] = (
                                    True
                                    if special_first in (special_bos, special_cls)
                                    else False
                                )
                                if special_first not in (special_bos, special_cls):
                                    logger.warning(
                                        f"Unknown leading special token {special_first!r} in TemplateProcessing<single>"
                                    )
                            if (
                                special_last := tmpl_single[-1]
                                .get("SpecialToken", {})
                                .get("id")
                            ):
                                if not tokenizer_config:
                                    ...
                                    tokenizer_config["eos_token"] = special_eos = (
                                        special_last
                                    )
                        ...
                        if tmpl_pair:
                            seq_start = (
                                1
                                if special_first
                                and tmpl_pair[0].get("SpecialToken", {}).get("id")
                                == special_first
                                else 0
                            )
                            seq_stop = (
                                -1
                                if special_last
                                and tmpl_pair[-1].get("SpecialToken", {}).get("id")
                                == special_last
                                else None
                            )
                                ...
                                if (
                                    ...
                                    if (
                                        ...
                                        if (
                                            ...
                                            add_sep = True
                                        if special_entry not in (
                                            special_sep,
                                            special_eos,
                                        ):
                                            logger.warning(
                                                f"Unknown separator token {special_entry!r} in TemplateProcessing<pair>"
                                            )
                                    else:
                                        logger.warning(
                                    ...
                                    if len(tmpl_pair) == 2:
                                        if (
                                            ...
                                            if special_entry not in (
                                                ...
                                                logger.warning(
                                                    f"Unknown second separator token {special_entry!r} in TemplateProcessing<pair>"
                                                )
                                        else:
                                            logger.warning(
                                                f"Unknown second middle sequence {tmpl_pair[1]!r} in TemplateProcessing<pair>"
                                            )
        ...
        if chat_template_jinja.is_file():
            ...
            if additional_templates := list(
                (path / "additional_chat_templates").glob("*.jinja")
            ):
...
class MistralVocab(Vocab):
    ...
    def __init__(self, base_path: Path):
        ...
        assert _filter_valid_tokenizer_files is not None, (
        ...
        if get_one_valid_tokenizer_file is not None:
            ...
            if len(valid_tokenizer_files) == 0:
                raise ValueError(
                    f"No tokenizer file found in the directory: {base_path}"
                )
    ...
    def get_token_id(self, token: str) -> int:
        assert SentencePieceTokenizer is not None and Tekkenizer is not None, (
            "mistral_common is not installed"
        )
