...
from .algo_phoneme import Phoneme, get_phoneme_seq
...
def _expand_by_words(
    text: str, start: int, end: int, expand_count: int = 1
) -> Tuple[int, int]:
    """按单词数量向左右扩展片段"""
...
def _extract_continuous_fragment(
    bounds: List[Tuple[int, int, str]], start_idx: int, end_idx: int, original_text: str
) -> str:
...
def extract_diff_fragments(
    wrong: str, right: str, zh_min_phonemes: int = 4, expand_words: int = 1
) -> List[str]:
    """
    ...
    matcher = SequenceMatcher(
        None, [b[2] for b in wrong_bounds], [b[2] for b in right_bounds]
    )
...
class RectificationRAG:
    ...
    def load_history(self):
        ...
        try:
            ...
            for block in blocks:
                ...
                if len(valid_lines) >= 2:
                    ...
                    if wrong and right:
                        ...
                        logger.debug(
                            f"加载纠错: '{wrong}' => '{right}', 检索词: {fragments}"
                        )
            ...
            if count > 0:
                ...
                logger.debug(
                    f"已加载 {count} 条纠错历史，{total_fragments} 个检索片段，耗时 {time.time() - start_time:.3f}s"
                )
    ...
    def _score_record(
        self, input_phonemes: List[Phoneme], record: RectifyRecord
    ...
    def format_prompt(
        self, text: str, top_k: int = 5, prefix: str = "纠错历史："
    ) -> str:
        """
        生成提示词
        ...
        """
        logger.debug(
            f"纠错 RAG format_prompt 调用: text='{text}', top_k={top_k}, 已加载记录数={len(self.records) if self.records else 0}"
        )

        if not self.records:
            logger.debug("纠错 RAG: 没有已加载的纠错记录")
            return ""

        results = self.search(text, top_k=top_k)
        if not results:
            logger.debug("纠错 RAG: 未检索到相关纠错历史")
            return ""
        ...
        if results:
            for r in results:
                ...
                for frag in r["fragments"]:
                    print(
                        f"    - '{frag['fragment']}' 得分={frag['score']} 音素数={frag['phonemes']}"
                    )
        else:
            # 即使没有匹配，也显示所有记录的得分
