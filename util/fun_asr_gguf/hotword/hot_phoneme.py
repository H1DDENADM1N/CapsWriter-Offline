...
class PhonemeCorrector:
    ...
    def _find_matches(
        self, text: str, fast_results: List, input_processed: List[Tuple]
    ...
    def _resolve_and_replace(
        self, text: str, matches: List[MatchResult]
        ...
        return (
            "".join(result_list),
            [(text[m.start : m.end], m.hotword, m.score) for m in final_matches],
            all_matched_info,
        )
    ...
    def correct(self, text: str, k: int = 10) -> CorrectionResult:
        ...
        return CorrectionResult(
            text=new_text, matchs=final_hw_info, similars=similars[:k]
        )
