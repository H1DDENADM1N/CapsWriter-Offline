...
def char_level_substring_score(main_text: str, pattern: str) -> float:
    ...
    for window_size in range(
        pattern_len, min(len(main_text) + 1, int(pattern_len * 1.5) + 1)
    ):
...
def find_best_match(
    ...
    logger.debug(
        f"[DEBUG] find_best_match: main_seq type={type(main_seq)}, len={len(main_seq)}"
    )
    logger.debug(
        f"[DEBUG] find_best_match: sub_seq type={type(sub_seq)}, len={len(sub_seq)}"
    )
    if main_seq:
        logger.debug(
            f"[DEBUG] find_best_match: main_seq[0] type={type(main_seq[0])}, value={main_seq[0]}"
        )
    if sub_seq:
        logger.debug(
            f"[DEBUG] find_best_match: sub_seq[0] type={type(sub_seq[0])}, value={sub_seq[0]}"
        )
    ...
    try:
        ...
        logger.error(
            f"[ERROR] main_seq details: {[(type(item), item) for item in main_seq[:5]]}"
        )
        raise
