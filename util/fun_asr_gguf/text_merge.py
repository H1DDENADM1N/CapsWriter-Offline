...
def merge_transcription_results(
    results: List[Dict[str, Any]], segment_offsets: List[float], overlap_s: float
    ...
    for i, res in enumerate(results):
        ...
        if i == 0:
            full_segments.extend(
                [
                    {"char": s["char"], "start": s["_global_start"]}
        ...
        curr_overlap_segs = [
            s for s in curr_segments if s["start"] <= curr_overlap_limit
        ]
        ...
        match = sm.find_longest_match(
            0, len(buffer_overlap_text), 0, len(curr_overlap_text)
        )
        ...
        if match.size >= 2:  # 至少匹配上 2 个字符
            ...
            try:
                ...
                for idx in range(len(full_segments) - 1, -1, -1):
                    if (
                        full_segments[idx]["start"] == target_seg["start"]
                        and full_segments[idx]["char"] == target_seg["char"]
                    ):
                        global_idx = idx
                        break
            ...
            if match_idx_in_curr != -1:
                ...
                full_segments.extend(
                    [{"char": s["char"], "start": s["_global_start"]} for s in to_add]
                )
            else:
                # 几乎不可能
                full_segments.extend(
    ...
    for s in full_segments:
        if (
            clean_segments
            and s["char"] in puncs
            and clean_segments[-1]["char"] == s["char"]
        ):
            continue
