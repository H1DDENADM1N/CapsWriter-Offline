...
from ..nano_dataclass import RecognitionStream, Statistics, TranscriptionResult
...
class TranscriptionOrchestrator:
    ...
    def __init__(self, models: ModelManager, logger: Optional[Logger] = None):
        self.models = models
        self.decoder = StreamDecoder(models, logger=logger)
        self.logger = logger
        ...
        with DisplayReporter(verbose=verbose) as reporter:
            try:
                ...
                if audio_duration <= segment_size + 2.0:
                    self._transcribe_short(
                        audio, result, language, context, verbose, reporter, base_offset
                    )
                else:
                    self._transcribe_long(
                        audio,
                        result,
                        language,
                        context,
                ...
                if srt and result.segments:
                    ...
                    reporter.print(
                        f"✓ 字幕已导出至: {os.path.basename(srt_path)}", force=True
                    )
    ...
    def _transcribe_short(
        self, audio, result, language, context, verbose, reporter, base_offset
    ):
        ...
        for seg in d_res.aligned or []:
            result.segments.append(
                {"char": seg["char"], "start": seg["start"] + base_offset}
            )
        ...
        if verbose:
            self._print_performance_stats(
                ...
                d_res,
                audio,
                result.timings.inject,
                result.timings.llm_generate,
            )
    ...
    def _transcribe_long(
        ...
        reporter.print("    检测到长音频，开启分段识别模式...", force=True)
        ...
        for idx, (s_s, e_s) in enumerate(segments_info):
            ...
            chunk = audio[
                int(s_s * self.models.config.sample_rate) : int(
                    e_s * self.models.config.sample_rate
                )
            ]
            stream = RecognitionStream()
            ...
            segment_results.append(
                {
                    ...
                    "ctc_text": "".join([r.text for r in d_res.ctc_results])
                    if d_res.ctc_results
                    else "",
                }
            )
        ...
        full_text, full_segs = merge_transcription_results(
            segment_results, offsets, overlap
        )
    ...
    def _print_performance_stats(self, reporter, d_res, audio, t_inject, t_llm):
        ...
        if t_inject > 0:
            stats.tps_in = stats.n_input_tokens / t_inject
        if t_llm > 0:
            stats.tps_out = d_res.n_gen / t_llm
        reporter.print(f"\n[统计]\n{stats}")
