...
class CTCDecoder:
    ...
    def decode(
        ...
        # 2. Decoding
        t0 = time.perf_counter()
        ctc_text, ctc_results, ctc_details = decode_ctc(
            ctc_logits, self.models.ctc_id2token
        )
...
class LLMDecoder:
    ...
    def decode(
        ...
        asr_decoder = llama.ASRStreamDecoder(
...
class StreamDecoder:
    ...
    def decode_stream(
        ...
        audio_embd, enc_output = encode_audio(
            stream.audio_data, self.models.encoder_sess, logger=self.logger
        )
        ...
        ctc_results, hotwords, ctc_times = self.ctc_decoder.decode(
            enc_output, self.models.config.enable_ctc, self.models.config.max_hotwords
        )
        ...
        for attempt in range(2):
            ...
            # 确保属性已初始化
            if not hasattr(timings, "prepare"):
                timings.prepare = 0.0
            ...
            # 已经解码第二遍则跳出
            if attempt == 1 or not (
                self.models.corrector and self.models.corrector.hotwords
            ):
                break

            # 重打分：使用 LLM 的结果进行热词匹配
            res_pass1 = self.models.corrector.correct(
                text, k=self.models.config.max_hotwords
            )

            # 收集 Pass 1 找到的所有热词
            pass1_hotwords = set()
            for _, hw, _ in res_pass1.matchs:
                pass1_hotwords.add(hw)
            for _, hw, _ in res_pass1.similars:
                pass1_hotwords.add(hw)
        ...
        if aligned:
            ...
            preview = " ".join(
                [f"{r['char']}({r['start']:.2f}s)" for r in aligned[:10]]
            )
            if len(aligned) > 10:
                preview += " ..."
