...
def set_llama_logger(logger_instance):
...
def _bind_function(lib, name, argtypes=None, restype=None):
    """安全地绑定DLL函数，添加详细日志"""
    try:
        ...
        _logger.info(f"Successfully bound: {name}")
        return func
    except AttributeError as e:
        _logger.error(f"Failed to bind function {name}: {e}")
        raise RuntimeError(f"DLL function {name} not found: {e}")
...
def init_llama_lib():
    ...
    _logger.info(f"Loading DLLs from: {lib_dir}")

    # 加载DLL
    try:
        ggml_path = os.path.join(lib_dir, GGML_DLL)
        _logger.info(f"Loading: {ggml_path}")
        ggml = ctypes.CDLL(ggml_path)
        _logger.info("ggml.dll loaded successfully")
    except Exception as e:
        _logger.error(f"Failed to load ggml.dll: {e}")
        raise

    try:
        ggml_base_path = os.path.join(lib_dir, GGML_BASE_DLL)
        _logger.info(f"Loading: {ggml_base_path}")
        ggml_base = ctypes.CDLL(ggml_base_path)
        _logger.info("ggml-base.dll loaded successfully")
    except Exception as e:
        _logger.error(f"Failed to load ggml-base.dll: {e}")
        raise

    try:
        llama_path = os.path.join(lib_dir, LLAMA_DLL)
        _logger.info(f"Loading: {llama_path}")
        llama = ctypes.CDLL(llama_path)
        _logger.info("llama.dll loaded successfully")
    except Exception as e:
        _logger.error(f"Failed to load llama.dll: {e}")
        raise
    ...
    llama_log_set = _bind_function(
        llama, "llama_log_set", [LOG_CALLBACK, ctypes.c_void_p], None
    )
    ...
    # 加载后端
    try:
        _logger.info("Loading ggml backend...")
        ggml_backend_load_all = _bind_function(ggml, "ggml_backend_load_all", [], None)
        ggml_backend_load_all()
        _logger.info("ggml backend loaded successfully")
    except Exception as e:
        _logger.error(f"Failed to load ggml backend: {e}")
        raise

    try:
        _logger.info("Initializing llama backend...")
        llama_backend_init = _bind_function(llama, "llama_backend_init", [], None)
        llama_backend_init()
        _logger.info("llama backend initialized successfully")
    except Exception as e:
        _logger.error(f"Failed to initialize llama backend: {e}")
        raise

    # 绑定其他函数
    _logger.info("Binding core llama functions...")

    llama_backend_free = _bind_function(llama, "llama_backend_free", [], None)
    llama_model_default_params = _bind_function(
        llama, "llama_model_default_params", [], llama_model_params
    )
    llama_model_load_from_file = _bind_function(
        llama,
        "llama_model_load_from_file",
        [ctypes.c_char_p, llama_model_params],
        ctypes.c_void_p,
    )
    llama_model_free = _bind_function(
        llama, "llama_model_free", [ctypes.c_void_p], None
    )
    llama_model_get_vocab = _bind_function(
        llama, "llama_model_get_vocab", [ctypes.c_void_p], ctypes.c_void_p
    )
    llama_model_n_embd = _bind_function(
        llama, "llama_model_n_embd", [ctypes.c_void_p], ctypes.c_int32
    )

    _logger.info("Core llama_model functions bound successfully")

    # Context
    _logger.info("Binding llama_context functions...")
    llama_context_default_params = _bind_function(
        llama, "llama_context_default_params", [], llama_context_params
    )
    llama_init_from_model = _bind_function(
        llama,
        "llama_init_from_model",
        [ctypes.c_void_p, llama_context_params],
        ctypes.c_void_p,
    )
    llama_free = _bind_function(llama, "llama_free", [ctypes.c_void_p], None)

    _logger.info("llama_context functions bound successfully")

    # Batch
    _logger.info("Binding llama_batch functions...")
    llama_batch_init = _bind_function(
        llama,
        "llama_batch_init",
        [ctypes.c_int32, ctypes.c_int32, ctypes.c_int32],
        llama_batch,
    )
    llama_batch_free = _bind_function(llama, "llama_batch_free", [llama_batch], None)

    _logger.info("llama_batch functions bound successfully")

    # Decode
    _logger.info("Binding llama_decode...")
    llama_decode = _bind_function(
        llama, "llama_decode", [ctypes.c_void_p, llama_batch], ctypes.c_int32
    )

    # Logits
    _logger.info("Binding llama_get_logits and llama_get_embeddings...")
    llama_get_logits = _bind_function(
        llama, "llama_get_logits", [ctypes.c_void_p], ctypes.POINTER(ctypes.c_float)
    )
    llama_get_embeddings = _bind_function(
        llama, "llama_get_embeddings", [ctypes.c_void_p], ctypes.POINTER(ctypes.c_float)
    )

    # Tokenize
    _logger.info("Binding llama_tokenize...")
    llama_tokenize = _bind_function(
        llama,
        "llama_tokenize",
        [
            ctypes.c_void_p,
    ...
    # Vocab
    _logger.info("Binding llama_vocab functions...")
    llama_vocab_n_tokens = _bind_function(
        llama, "llama_vocab_n_tokens", [ctypes.c_void_p], ctypes.c_int32
    )
    llama_vocab_eos = _bind_function(
        llama, "llama_vocab_eos", [ctypes.c_void_p], llama_token
    )
    llama_token_to_piece = _bind_function(
        llama,
    ...
    _logger.info("llama_vocab functions bound successfully")

    # Memory (KV Cache)
    _logger.info("Binding llama_memory functions...")
    llama_get_memory = _bind_function(
        llama, "llama_get_memory", [ctypes.c_void_p], ctypes.c_void_p
    )
    llama_memory_clear = _bind_function(
        llama, "llama_memory_clear", [ctypes.c_void_p, ctypes.c_bool], None
    )

    _logger.info("llama_memory functions bound successfully")

    # Sampler
    _logger.info("Binding llama_sampler functions...")
    try:
        llama_sampler_chain_default_params = _bind_function(
            llama, "llama_sampler_chain_default_params", [], llama_sampler_chain_params
        )
        llama_sampler_chain_init = _bind_function(
            llama,
            "llama_sampler_chain_init",
            [llama_sampler_chain_params],
            ctypes.c_void_p,
        )
        llama_sampler_chain_add = _bind_function(
            llama, "llama_sampler_chain_add", [ctypes.c_void_p, ctypes.c_void_p], None
        )
        llama_sampler_init_greedy = _bind_function(
            llama, "llama_sampler_init_greedy", [], ctypes.c_void_p
        )
        llama_sampler_init_dist = _bind_function(
            llama, "llama_sampler_init_dist", [ctypes.c_uint32], ctypes.c_void_p
        )
        llama_sampler_init_temp = _bind_function(
            llama, "llama_sampler_init_temp", [ctypes.c_float], ctypes.c_void_p
        )
        llama_sampler_init_top_k = _bind_function(
            llama, "llama_sampler_init_top_k", [ctypes.c_int32], ctypes.c_void_p
        )
        llama_sampler_init_top_p = _bind_function(
            llama,
            "llama_sampler_init_top_p",
            [ctypes.c_float, ctypes.c_size_t],
            ctypes.c_void_p,
        )
        llama_sampler_sample = _bind_function(
            llama,
            "llama_sampler_sample",
            [ctypes.c_void_p, ctypes.c_void_p, ctypes.c_int32],
            llama_token,
        )
        llama_sampler_free = _bind_function(
            llama, "llama_sampler_free", [ctypes.c_void_p], None
        )
        llama_sampler_init_logit_bias = _bind_function(
            llama,
            "llama_sampler_init_logit_bias",
            [ctypes.c_int32, ctypes.c_int32, ctypes.POINTER(llama_logit_bias)],
            ctypes.c_void_p,
        )
        _logger.info("llama_sampler functions bound successfully")
    except RuntimeError as e:
        # 版本较旧的 llama.cpp 可能没有这些导出
        _logger.warning(f"llama.cpp 库中缺少某些采样 API: {e}")
        _logger.warning("将无法使用原生采样优化。")
...
def load_model(model_path: str):
    ...
    _logger.info(f"Changed directory to: {Path.cwd()}")

    # 初始化 backend，载入模型
    init_llama_lib()
    model_params = llama_model_default_params()
    model = llama_model_load_from_file(
        model_rel.as_posix().encode("utf-8"), model_params
    )

    if model:
        os.chdir(original_cwd)
        _logger.info(f"Restored directory to: {Path.cwd()}")
        return model
    else:
        _logger.error(f"当前路径：{Path.cwd()}")
        _logger.error(f"模型绝对路径：{model_path.as_posix()}")
        _logger.error(f"模型可访问性：{model_path.exists()}")
        _logger.error(f"模型加载失败: {model_path}")
        return None
...
def create_context(
    ...
    )  # 1 = ON, 0 = OFF (auto typically uses what's available)
...
class LlamaBatch:
    ...
    @property
    def n_tokens(self):
        return self.struct.n_tokens

    @n_tokens.setter
    def n_tokens(self, val):
        self.struct.n_tokens = val

    @property
    def token(self):
        return self.struct.token

    @property
    def embd(self):
        return self.struct.embd
    ...
    @property
    def n_seq_id(self):
        return self.struct.n_seq_id

    @property
    def seq_id(self):
        return self.struct.seq_id

    @property
    def logits(self):
        return self.struct.logits
...
class LlamaSampler:
    ...
    def __init__(
        ...
        if logit_bias and n_vocab > 0 and isinstance(logit_bias, dict):
            ...
            llama_sampler_chain_add(
                self.ptr, llama_sampler_init_logit_bias(n_vocab, n_bias, bias_data)
            )
    ...
    def sample(self, ctx, idx=-1, limit_start=None, limit_end=None):
        ...
        if (
            (limit_start is not None or limit_end is not None)
            and hasattr(ctx, "get_logits")
            and hasattr(ctx, "model")
        ):
            # 需要获取 n_vocab
...
def python_log_callback(level, message, user_data):
    ...
    try:
        ...
        if "error" in msg_str.lower() or "failed" in msg_str.lower():
            _logger.error(f"[llama.cpp] {msg_str}")
        elif "warn" in msg_str.lower():
            _logger.warning(f"[llama.cpp] {msg_str}")
        elif level == 2:
            _logger.info(f"[llama.cpp] {msg_str}")
        elif level == 3:
            _logger.warning(f"[llama.cpp] {msg_str}")
        elif level == 4:
            _logger.info(f"[llama.cpp] {msg_str}")
        elif level >= 5:
            _logger.debug(f"[llama.cpp] {msg_str}")
        else:
            _logger.info(f"[llama.cpp] {msg_str}")
...
def get_token_embeddings_gguf(model_path):
    ...
    if target_tensor.tensor_type == 8:  # Q8_0
        ...
        if isinstance(data, np.memmap) or isinstance(data, np.ndarray):
            ...
            # 兜底：如果是原始 buffer
            data = (
                np.frombuffer(target_tensor.data, dtype=np.float16)
                .astype(np.float32)
                .reshape(-1, n_embd)
            )
